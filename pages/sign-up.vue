...
async function handleFormSubmit() {
  ...
  if (response.status === 'success') {
    toast.add({ title: 'Готово!', description: 'Вы успешно зарегистрировались', color: 'success' });
    resetForm();
/*    await navigateTo({
      path: '/sign-in',
      query: {
...
<template>
  <UForm @submit="handleFormSubmit" :state="state" class="flex flex-col mx-auto justify-center max-w-md mt-10 p-6 rounded-lg shadow-md">
    ...
    <UFormField label="Username" name="username" size="xl" class="mb-6">
      <UInput
          class="w-full"
